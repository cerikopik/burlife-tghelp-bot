...
async def button_handler(update, context):
    ...
    try:
        if data.startswith("reply_to_"):
            ...
            preset_message = "Требуется дополнительная проверка вашей личности 🕵️‍♀️"
            "Ваша заявка была отклонена администратором чтобы убедиться, что вы являетесь сотрудником компании.\n\n"
            "Просто отправите боту фотографию 🖼️, например вашего пропуска или удостоверения о проверке знаний, где будут четко видны ваши Имя и Фамилия."
            "‼️Ни в коем случае не присылаете фото вашего паспорта или водительского удостоверения."
            await context.bot.send_message(
                chat_id=user_id,
                text=preset_message
            )
