...
async def reply_to_user(update, context):
    ...
    if replied_to_msg.from_user.is_bot and replied_to_msg.text:
        ...
        if match:
            ...
            preset_text = "📧Ваша [ПРИГЛАСИТЕЛЬНАЯ ССЫЛКА](https://t.me/+k2dfZY9KPAowNjM6) в телеграм канал. После перехода по ссылке нажмите подать заявку, и она будет одобрена автоматически 👌"
            await context.bot.send_message(
                chat_id=user_id,
                text=preset_text,
                parse_mode='Markdown'
